...
from table2rules.exporters import DEFAULT_FORMAT, available_exporters
...
def run_case(case_path: Path, fmt: str) -> str:
    html = case_path.read_text(encoding="utf-8")
    return process_tables_to_text(html, format=fmt)
...
def compare_case(case_path: Path, output: str, allow_missing_gold: bool, gold_dir: Path) -> CaseResult:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        help=f"Exporter to benchmark (default: {DEFAULT_FORMAT}). "
             "Gold files live under benchmarks/gold/<format>/.",
    )
...
def main() -> int:
    ...
    current_dir = CURRENT_DIR / args.format
    gold_dir = GOLD_DIR / args.format

    for case in cases:
        output = run_case(case, args.format)
        ...
        if args.update_gold:
            ...
            results.append(compare_case(case, output, args.allow_missing_gold, gold_dir))
